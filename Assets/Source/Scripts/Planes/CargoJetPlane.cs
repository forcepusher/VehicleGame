...
namespace BananaParty.VehicleGame
{
    public class CargoJetPlane : JetPlane
    {
        ...
        protected override float TakeoffVelocity => 20f;
        ...
        protected override Vector3 AccelerationParked => new Vector3(0f, 0f, 1.5f); // ~1.2G strong initial push from standstill
        protected override Vector3 AngularAccelerationParked => new Vector3(0f, 0f, 0f); // pitch: yaw: roll

        protected override Vector3 AccelerationTaxi => new Vector3(0f, 0f, 3f); // ~0.8G ground roll (clunky)
        protected override Vector3 AngularAccelerationTaxi => new Vector3(0f, 0.5f, 0f); // pitch: yaw: roll (near zero at low speed)

        protected override Vector3 AccelerationTakeoff => new Vector3(0f, 0f, 7f); // ~0.8G for takeoff acceleration
        protected override Vector3 AngularAccelerationTakeoff => new Vector3(1f, 1f, 1f); // pitch: yaw: roll (sluggish controls)

        protected override Vector3 AccelerationFight => new Vector3(0f, 0f, 15f); // ~1G at cruise speed (good airspeed buildup)
        protected override Vector3 AngularAccelerationFlight => new Vector3(3f, 3f, 3f); // pitch: yaw: roll (smoother in flight)
